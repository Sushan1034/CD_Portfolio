    ...
    <section id="about" className="py-24 relative overflow-hidden">
      <div className="section-container relative z-10">
        ...
        >
          ...
          <div className="relative overflow-x-auto md:overflow-visible pt-8 pb-6 md:pt-0 md:pb-0 hide-scrollbar w-full">
            <div className="flex md:grid md:grid-cols-7 relative z-20 min-w-max md:min-w-full h-32 md:h-16 gap-6 md:gap-0 px-6 md:px-0 w-full">
              {stages.map((stage, index) => {
                const isActive = index === currentStep;
                const isPast = index < currentStep;

                return (
                  <div key={stage.id} className="flex flex-col items-center relative group w-32 md:w-auto">
                    <div className="relative">
                      <motion.button
                        onClick={() => setCurrentStep(index)}
                        className={`w-12 h-12 md:w-16 md:h-16 rounded-2xl flex items-center justify-center text-xl transition-all duration-500 border-4 ${isActive
                    ...
                    <div className="hidden md:block mt-6 text-center max-w-[120px] mx-auto">
                      <p className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 whitespace-normal leading-relaxed ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}>
                        {stage.title}
                      </p>
                    </div>

                    {/* Mobile Labels */}
                    <div className="md:hidden mt-3 text-center w-full px-2">
                      <p className={`text-[10px] font-bold uppercase tracking-tight leading-tight min-h-[2.5rem] flex items-center justify-center ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400'}`}>
                        {stage.title}
                      </p>
                    </div>
                  </div>
